...
def handle_data(context, data):
    ...
    for stock in context.sids:
        ...
        if stock not in context.long and stock not in context.short and notional < context.max_notional and notional > context.min_notional:
            if mavg_fast > mavg_slow and rsi < 50 and mavg < data[stock].price:
                order(stock, 100)
                log.info("Long %f, %s" %(data[stock].price, stock))
                context.long[stock] = data[stock].price
            elif mavg_fast < mavg_slow and rsi > 50 and mavg > data[stock].price:
                order(stock, -100)
    ...
    record(EMA40=acc_fast/10, EMA80=acc_slow/10, RSI=acc_rsi/10, SMA200=acc_mavg/10, Price=acc_price/10)

@batch_transform(window_length=40)
def get_EMA_fast(datapanel):
    global Current_stock
    prices=datapanel['price']
    EMA = pandas.stats.moments.ewma(prices, span=40)
    return EMA[Current_stock][39]

@batch_transform(window_length=80)
def get_EMA_slow(datapanel):
    global Current_stock
    prices=datapanel['price']
    EMA = pandas.stats.moments.ewma(prices, span=80)
    return EMA[Current_stock][79]